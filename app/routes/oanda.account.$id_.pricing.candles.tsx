...
import { getCandles } from '~/.server/oanda/pricing';
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  const price = data.get('price') as PricingComponent;

  return getCandles({ accountID: params.id as AccountID, instrument, granularity, price });
};
...
const Candles = () => {
  ...
  return <div>
    <header un-grid='~ inline' un-gap='6' un-auto-flow='col' un-justify='start' un-items='center' >
      ...
      {
        fetcher.state === 'loading' && <div className="i-line-md:loading-loop" un-text='blue-600 3xl' ></div>
      }
      {
        fetcher.state === 'idle' && <Button type='primary'
          icon={<div className="i-ic:baseline-check" />}
          onClick={() => {
            fetcher.submit({ instrument, granularity, price: price.join('') }, { method: 'POST' });
          }}
        />
      }
    </header>
