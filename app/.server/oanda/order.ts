...
export const getPendingOrders = async ({ id, acceptDatetimeFormat, accountID }: { id: string; acceptDatetimeFormat?: AcceptDatetimeFormat; accountID?: AccountID; }) => {
  const response = await fetch(`${oandaUrl}/v3/accounts/${id}/pendingOrders`, {
    headers: {
      'Authorization': `Bearer ${process.env.OANDA_API_KEY ?? ''}`,
    },
  });
